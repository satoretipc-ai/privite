...
WASDCancelEnable := Integer(IniRead(ConfigFile, "Settings", "WASDCancelEnable", 0))
...
; ★WASD相殺機能
myGui.Add("Text","xs","WASD相殺:")
chkWASDCancel := myGui.Add("Checkbox","xs", "WASD相殺を有効にする")
chkWASDCancel.Value := WASDCancelEnable
...
A_TrayMenu.Add("一   停止切替", (*) => TogglePause())
A_TrayMenu.Add()
...
ApplySettings(){
    ...
    global WASDDelayMs, WASDDisable, WASDCancelEnable, FovX, FovY, SW, SH, SL, ST, SR, SB, TriggerMode
    ...
    global chkRandom, eMinHold, eMaxHold, eWASDDelay, chkWASDDisable, chkWASDCancel
    ...
    WASDCancelEnable := chkWASDCancel.Value
    ...
    IniWrite(WASDCancelEnable, ConfigFile, "Settings", "WASDCancelEnable")

    UpdateModeUI()

    wasdStatus := WASDDisable ? "有効" : "無効"
    cancelStatus := WASDCancelEnable ? "有効" : "無効"
    ToolTip("保存: " TriggerMode " / WASD中無効 " wasdStatus " / 相殺 " cancelStatus, 10, 10)
...
BeginHold(x, y){
    global _hChild, _hRoot, _lpChild, _lpRoot, WASDCancelEnable, _cancelKeysDown
    ...
    if WASDCancelEnable {
        ...
        if GetKeyState("w","P") {
            SendInput("{s down}")
            _cancelKeysDown["s"] := true
        }
        if GetKeyState("a","P") {
            SendInput("{d down}")
            _cancelKeysDown["d"] := true
        }
        if GetKeyState("s","P") {
            SendInput("{w down}")
            _cancelKeysDown["w"] := true
        }
        if GetKeyState("d","P") {
            SendInput("{a down}")
            _cancelKeysDown["a"] := true
        }
    }
...
KeepAlive(){
    global _hChild, _hRoot, _lpChild, _lpRoot, RepeatDownMs, WASDCancelEnable, _cancelKeysDown

    ; ★WASD相殺ロジック（持続中）
    if WASDCancelEnable {
        ; W → Sを押し続ける
        if GetKeyState("w","P") {
            if !_cancelKeysDown.Has("s") {
                SendInput("{s down}")
                _cancelKeysDown["s"] := true
            }
        } else {
            if _cancelKeysDown.Has("s") {
                SendInput("{s up}")
                _cancelKeysDown.Delete("s")
            }
        }
        
        ; A → Dを押し続ける
        if GetKeyState("a","P") {
            if !_cancelKeysDown.Has("d") {
                SendInput("{d down}")
                _cancelKeysDown["d"] := true
            }
        } else {
            if _cancelKeysDown.Has("d") {
                SendInput("{d up}")
                _cancelKeysDown.Delete("d")
            }
        }
        
        ; S → Wを押し続ける
        if GetKeyState("s","P") {
            if !_cancelKeysDown.Has("w") {
                SendInput("{w down}")
                _cancelKeysDown["w"] := true
            }
        } else {
            if _cancelKeysDown.Has("w") {
                SendInput("{w up}")
                _cancelKeysDown.Delete("w")
            }
        }
        
        ; D → Aを押し続ける
        if GetKeyState("d","P") {
            if !_cancelKeysDown.Has("a") {
                SendInput("{a down}")
                _cancelKeysDown["a"] := true
            }
        } else {
...
EndHold(){
    global _hChild, _hRoot, _lpChild, _lpRoot, _cancelKeysDown
